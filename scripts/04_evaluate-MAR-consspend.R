# Evaluate robustness of model parameter estimation to violation of ‘missing at random’ assumption for Conservation Spending indicator

library(tidyverse)

resid.df <- read.csv('outputs/predicted-indicator-vals/predicted-ind-vals-LV9-final.csv')
resid.df2 <- read.csv('outputs/predicted-indicator-vals/predicted-ind-vals-LV9-final-nospend.csv')
...
resid.df.long <- pivot_longer(select(resid.df, -mean_annual_funds), OECD_sc:ramsar.manage, 
...
resid.df.long2 <- pivot_longer(resid.df2, OECD_sc:ramsar.manage, 
...
a <- ggplot(dat.plot)+
  ...
  ylab('Predictions without conservation spending') +
  xlab('Predictions with conservation spending') +
  theme_classic()
a

ggsave('outputs/compare-predictions-cons-spending.png', width = 8, height = 8)
