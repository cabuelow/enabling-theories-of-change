...
dat.long <- dat %>%
  pivot_longer(cols = c(WGI_GovEff_est:NXA.ind.2020),
               names_to = 'indicator',
               values_to = 'val')
...
ynames
write.csv(data.frame(ynames), 'outputs/colnames-boral-nospend.csv', row.names = F)

family <- rep("normal", ncol(y.scale)) # first set all as normal
family[c(11, 12)] <- "poisson" # then change the poission by column numbers (count)
family[c(13, 14, 15, 18)] <- "binomial" # then change the binomial by column numbers (binary)
...
saveRDS(m1, paste0('outputs/models/mod-LV', i, '-final-nospend.rds'))

# plot DS residuals

pdf(paste0('outputs/diagnostics/DS-residuals-LV', i, '-final-nospend.pdf'))
plot(m1)
dev.off()
...
write.csv(df, paste0('outputs/diagnostics/gewecke-convergence-diagnostics-LV', i, '-final-nospend.csv'), row.names = F)

# check chain convergence with trace plots

pdf(paste0('outputs/diagnostics/mcmc-trace-LV', i, '-final-nospend.pdf'))
plot(get.mcmcsamples(m1))
dev.off()

# check autocorrelation

pdf(paste0('outputs/diagnostics/autocorrelation-LV', i, '-final-nospend.pdf'))
